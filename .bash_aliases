...
# loads env variables example below
...
# Usage: in the sshd config following lines must be set

###alias SOCKS5='ssh -p $INTEL_VM_PORT -C2qTnN  -D 8080 -i $INTEL_VM_KEY $INTEL_VM_USER@$INTEL_VM_ADRESS & ~/Documents/firefox/firefox'
###alias SSHFS_FS='shfs fs@$INTEL_VM_ADRESS:/sshfs ~/sshfs -C -p $INTEL_VM_PORT -o IdentityFile=~/.ssh/fs.key -odebug,sshfs_debug,loglevel=debug'
##
###alias PUSHBASHALIAS='scp -P $INTEL_VM_PORT -o IdentityFile=~/.ssh/vm.key ~/.bash_aliases ~/.bash_secrets user@$INTEL_VM_ADRESS:~/testfolder/'
#
#-------------------------------------------------------------
# EO related
#-------------------------------------------------------------
...
# alias zeit='date +%Y-%m-%d_T%H%M'
# alias llt='ls -tral'
# alias tailg='tail -f /var/adm/ras/mmfs.log.latest'
# alias lesslate='less /var/adm/ras/mmfs.log.latest'

alias netboxup='docker-compose -f ~/randomGIT/netbox-docker/docker-compose.yml up'
